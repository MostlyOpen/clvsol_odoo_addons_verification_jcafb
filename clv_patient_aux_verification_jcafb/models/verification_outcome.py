...
class PatientAux(models.Model):
    ...
    verification_marker_ids = fields.Many2many(
        ...
        relation='clv_patient_aux_verification_marker_rel',
        ...
        for patient_aux in self:
            ...
            for verification_template in verification_templates:
                ...
                if verification_outcome.state is False:
                    ...
                    verification_outcome_values['res_id'] = patient_aux.id
                    verification_outcome_values['res_last_update'] = patient_aux['__last_update']
                ...
                action_call = 'self.env["clv.verification.outcome"].' + \
                    ...
                    '(verification_outcome, patient_aux)'
            ...
            this_patient_aux = self.env['clv.patient_aux'].with_context({'active_test': False}).search([
                ('id', '=', patient_aux.id),
            ])
            VerificationOutcome._object_verification_outcome_model_object_verification_state_updt(this_patient_aux)

            this_patient_aux._compute_verification_outcome_infos()


class VerificationOutcome(models.Model):
    _inherit = 'clv.verification.outcome'

    def _patient_aux_verification(self, verification_outcome, model_object):
        ...
        if (model_object.is_absent is True) and (model_object.state not in ['unavailable']):
            ...
            outcome_info += _('"Patient (Aux) State" should be "Unavailable".\n')
            state = self._get_verification_outcome_state(state, 'Error (L0)')
    ...
    def _patient_aux_verification_related_patient(self, verification_outcome, model_object):
